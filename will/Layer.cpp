...
{
    int batch = option_->getInt("", "batch", 1);
    ...
    case LAYER_CONNECTION_FULLCONNECT:
    {
        ...
        op.as_mul(W_, prev_layer_->A_, A_);
    ...
    case LAYER_CONNECTION_CONVOLUTION:
    {
        ...
        op.as_conv(prev_layer_->A_, W_, A_, stride, padding);
        op_queue.push_back(op);
        if (option_->getInt(layer_name_, "need_bias", 1))
        {
            MatrixOp op;
    ...
    case LAYER_CONNECTION_COMBINE:
    {
        ...
        if (option_->getEnum<CombineType>(layer_name_, "type") == COMBINE_CONCAT)
        {
            op.as_concat(in, A_);
        }
        else
