...
public:
    ...
    //以下为枚举值的处理
private:
    static std::map<std::string, std::map<std::string, int, CompareNoUnderline>> enum_map_;
    static std::map<std::string, std::map<int, std::string>> enum_map_reverse_;
    //注册枚举值
    template <typename T>
