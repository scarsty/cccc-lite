...
| rocm-cmake | CMake 模块，位于 `C:\project\build-tools\rocm-cmake` |
| BZip2 | 通过 vcpkg 安装（`bzip2:x64-windows`），CMake 自动发现 |
| nlohmann_json | 通过 vcpkg 安装（`nlohmann-json:x64-windows`），header-only |
| Half | 通过 vcpkg 安装（`half:x64-windows`），header-only |
...
```powershell
$vcpkg = "C:\project\microsoft\vcpkg"
# frugally-deep/eigen3/functionalplus 为 AI tuning header-only 依赖
# bzip2/nlohmann-json/half 由 CMake 通过 PREFIX_PATH 自动发现
& "$vcpkg\vcpkg.exe" install frugally-deep:x64-windows eigen3:x64-windows bzip2:x64-windows nlohmann-json:x64-windows half:x64-windows
...
```powershell
$dir = "C:\project\build-tools\amd_comgr-cmake"
...
$prefix = "C:/project/build-tools/rocm-cmake;C:/Program Files/AMD/ROCm/7.1;C:/Program Files/AMD/ROCm/7.1/hip;C:/project/microsoft/vcpkg/installed/x64-windows"
...
& $cmake -G Ninja -S . -B build `
  ...
  -Damd_comgr_DIR="C:/project/build-tools/amd_comgr-cmake" `
  ...
  -DUNZIPPER="C:/project/microsoft/vcpkg/installed/x64-windows/tools/bzip2/bzip2.exe"
```
...
| CMake 报错 "Could not find amd_comgr-config.cmake" | ROCm 7.1 安装包遗漏 cmake 包配置文件（ROCm 6.x 有，7.1 无）| 手动创建 4 个 cmake 文件到 `C:\project\build-tools\amd_comgr-cmake\`，cmake 加 `-Damd_comgr_DIR`（见步骤四）|
