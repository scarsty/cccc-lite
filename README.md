...
| 模型 | 参数量 | 精度 | VRAM | decode 速度（RTX 5090） |
|------|--------|------|------|------------------------|
| Qwen3-8B | 8B | BF16 | ~18 GB | ~34 tok/s |
| Qwen3-8B W8A16 | 8B | FP8_E5M2 权重 + BF16 激活 | ~10 GB | ~17 tok/s |
| Hy-MT2-7B（混元机器翻译） | 7B | BF16 | ~18 GB | ~14 tok/s |
| Hy-MT2-7B W8A16 | 7B | FP8_E5M2 权重 + BF16 激活 | ~12 GB | ~17 tok/s |
| Llama 3.1-8B FP4 | 8B | FP4_E2M1 block-scale 权重 + BF16 激活 | ~12.5 GB | ~12 tok/s |

Qwen3 支持思维链（thinking）模式；Hy-MT2-7B 为专用翻译模型（GQA，use_qk_norm，tie_word_embeddings）。W8A16 格式权重量化为 FP8/FP4，激活保留 BF16，显存占用约为 BF16 的一半。
