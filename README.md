...
cccc同时支持大语言模型（LLM）的推理，内置字节级BPE分词器（兼容Qwen3等主流模型），支持多轮对话、思维链过滤和流式逐词输出。

## 大语言模型推理

cccc通过cccc-llm动态库（cccc-llm.dll）提供LLM推理接口，由cccc-windows调用，实现交互式对话。

目前已验证支持 Qwen3 系列模型（如 Qwen3-0.5B）。需先将Hugging Face格式的权重转换为cccc格式，已转换好的权重在 <https://huggingface.co/scarsty/cccc-llm>。

### 运行示例

```shell
# 交互式多轮对话
cccc-windows --llm -c work\qwen3\qwen3_0.5b.ini
```

qwen3目录下应有tokenizer.json文件。

## 图像生成（Diffusion Model）

cccc通过cccc-sd动态库（cccc-sd.dll）提供扩散模型图像生成接口，支持文本到图像（Text-to-Image）的推理。
